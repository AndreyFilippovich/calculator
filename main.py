...
class Calculator(QMainWindow):
    ...
    def add_digit(self):
        ...
        Сигнал- это нажатие кнопки,
        то есть в переменной будет хранится последняя нажатая кнока
        Чтобы понять, что нажатая кнопка- это цифра,
        создаем кортеж с именами кнопок цифр
        Если имя объекта, которое послало сигнал есть в этом кортеже,
        значит выполняем тот самый метод добавления цифры
        """
    ...
    # возвращает только строку или ничего
    def get_math_sign(self) -> Optional[str]:
        """Метод для получения знака из временного выражения.
        Сначала удостоверимся о том, что в лейбле есть текст,
        затем получаем текст из него,
        разделяем по пробелам и вытаскиваем последний элемент.
        """
        if self.ui.lbl_temp.text():
    ...
    def adjust_temp_font_size(self) -> None:
        ...
        while self.get_temp_text_width() > self.ui.lbl_temp.width() - 15:
            ...
            self.ui.lbl_temp.setStyleSheet(
                f'font-size: ' + str(font_size) + 'pt; color: #888;')
